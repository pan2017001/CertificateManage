jQuery.bootstrapLoading = {
    start: function (options) {
        ...
        var _LoadingHtml = '<div id="loadingPage" align="center" style="position:fixed;left:0;top:0;_position: absolute;width:100%;height:' + _PageHeight + 'px;background:' + options.backgroundColor + ';opacity:' + options.opacity *0.6 + ';filter:alpha(opacity=' + options.opacity + ');z-index:' + options.zindex + ';"><div id="loadingTips" align="center" style="position: absolute; cursor1: wait; width: 40% ;border-color:' + options.borderColor + ';border-style:' + options.borderStyle + ';border-width:' + options.borderWidth + 'px; height:80px; line-height:80px; padding-left:80px; padding-right: 5px;border-radius:10px;  background: ' + options.backgroundColor + ' url(/Content/bootstrap-loading/images/loading.gif) no-repeat 5px center; color:' + options.TipsColor + ';font-size:20px;">' + options.loadingTips + '</div></div>';
